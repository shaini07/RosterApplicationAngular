...
angular
    .module('searchAllowanceCtrl', ['angularjs-datetime-picker', 'ngStorage', 'ui.bootstrap', 'ngLoader'])
	.controller('SearchAllowanceController', ['$rootScope', '$scope', '$localStorage', '$http', '$filter', '$sessionStorage', '$route', '$timeout', function ($rootScope, $scope, $localStorage, $http, $filter, $sessionStorage, $route, $timeout) {
	    ...
	    if (checkSessionStorage($sessionStorage, $scope, $localStorage, $rootScope)) {
	        ...
	        pageInitialize($scope, $rootScope, $localStorage, 'SearchAllowance');
	        //Loading the loader
	        $scope.working = true;
	        //Loading Shift Names
	        ...
	        $scope.submit = function () {
	            ...
	            $timeout(function () { loadAllowance($scope, $http, $sessionStorage); $scope.working = false; }, 2000);
	        };
	        ...
	        //Delete Allowance
	        $scope.deleteAllowance = function (allowanceId) {
	            //Loading the loader
	            $scope.working = true;
	            //Loading Roaster search grid	
	            $timeout(function () {
	                deleteRecord(allowanceId, $route); $scope.working = false;
	            }, 2000);

	        };
	        //Export the Shift details
	        var mystyle = {
	            sheetid: 'Shift Allowance',
	            headers: true,
	            caption: {
	            ...
	            columns: [
                    ...
                    { columnid: 'Allowance', title: 'Allowance' },
...
//Delete Allowance function
function deleteRecord(allowanceId, $route) {
            ...
            if (caption == "Yes") {
                ...
                if (responseData != null) {
                    if (responseData.length == 0) {
                        successMessge('Record deleted succesfully', 'Delete Allowance', '#/SearchAllowance');
                        $route.reload();
...
//Load Allowance Service
function loadAllowance($scope, $http, $sessionStorage) {
